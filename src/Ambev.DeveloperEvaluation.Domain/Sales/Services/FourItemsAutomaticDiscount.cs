...
public class FourItemsAutomaticDiscount(ILogger<FourItemsAutomaticDiscount> logger, IDiscountRepository repository)
    : IAutomaticDiscount
{
    private static readonly DiscountCode _discountCode = DiscountCode.Auto4Items;
    ...
    private static bool CanApply(Sale sale) =>
        sale.Items.Any(i => i.Quantity.Value is >= 4 and < 10) && sale.Items.All(i => i.Quantity.Value < 10);
}
